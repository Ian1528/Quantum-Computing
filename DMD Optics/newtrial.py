...
class qtwindow():
    def __init__(self):
        ...
        print("Dynamic library path:", lib_path)
        #   类变量
        self.Device_ID = 0
    ...
    def on_pushButton_CMD_CX_clicked(self):
        ...
        if res != 0:
            ...
            print("Failed to send query command, failure code:" + str(res) + "\n", 1)
        else:
            print("Succeeded in sending query command\n", 0)
    ...
    def on_pushButton_CMD_DeviceInfo_clicked(self):
        ...
        res = self.lib.FL_DLP_HS_AnalysisDevice(self.Device_ID, self.Thread_receive.XC_Data, DeviceInfo,
                                                byref(DeviceInfolen))
        if res != 0:
            print("Failed to load device information, failure code:" + str(res) + "\n", 1)
        else:
            print("Successfully loaded device information\n", 0)
    ...
    # 设置参数
    def on_pushButton_CMD_SetParam_clicked(self,Param_Inverse,Param_Delay,Param_Gray = 1,Param_CFPicnum = 1,Param_CFDelay = 1000):
        # Param_Delay = int(self.lineEdit_Param_Delay.text())，单位是Hz
        ...
        if res != 0:
            print("Failed to send setting parameters, failure code:" + str(res) + "\n", 1)
        else:
            print("Sending setting parameters succeeded\n", 0)
    ...
    # 二值图数组Demo
    def on_pushButton_SendChar_clicked(self,bit_array,position):
        width = 1280
        height = 800
        ...
        for row in bit_array:
            for bit in row:
                # 每次处理一个bit，左移并加入当前bit
                byte = (byte << 1) | bit
                bit_count += 1

                # 每8个bit组成一个字节，转换为十六进制并追加到结果字符串
                if bit_count == 8:
                    picchar_s += f"{byte:02x}"  # 格式化为两位十六进制数
                    byte = 0  # 重置字节
                    bit_count = 0  # 重置bit计数器

        picchar_b = bytes(bytearray.fromhex(picchar_s))
        picchar2 = cast(picchar_b, c_char_p)
        print(picchar_s)
        res = self.lib.FL_DLP_HS_Send_PICDATA(self.Device_ID, 4, None, 1, position, picchar2)
        if res != 0:
            print("发送二值图数组Demo失败，失败代码：" + str(res) + "\n", 1)
        else:
            print("发送二值图数组Demo成功\n", 0)
    # 设置序列
    def on_pushButton_CMD_SetSequence_clicked(self):
        ...
        if res != 0:
            print("Failed to send setting sequence, failure code:" + str(res) + "\n", 1)
        else:
            print("Succeeded in sending setting sequence\n", 0)


    # 播放
    def on_pushButton_CMD_Play_clicked(self,type1,Param_StartP,Param_PlayPicnum):
        """
        Send command to play the sequence.

        Args:
            type1 (_type_): 1 = single, 2 = loop
            Param_StartP (_type_): which picture to start from
            Param_PlayPicnum (_type_): number of pictures to play
        """
        ...
        if res != 0:
            print("Failed to send play command, failure code:" + str(res) + "\n", 1)
        else:
            print("Successfully sent the play command\n", 0)
    ...
    def on_pushButton_CMD_PlayStop_clicked(self):
        ...
        if res != 0:
            print("Failed to send pause command, failure code:" + str(res) + "\n", 1)
        else:
            print("Successfully sent pause command\n", 0)
    ...
    def on_pushButton_CMD_DMDFloat_clicked(self):
        ...
        res = self.lib.FL_DLP_HS_Send_Fixed_Cmd_Noparam(self.Device_ID, type1)
        if res != 0:
            print("发送DMD-Float命令失败，失败代码：" + str(res) + "\n", 1)
        else:
            print("发送DMD-Float命令成功\n", 0)
    ...
    def on_pushButton_CMD_Reset_clicked(self):
        ...
        res = self.lib.FL_DLP_HS_Send_Fixed_Cmd_Noparam(self.Device_ID, type1)
        if res != 0:
            print("发送DMD-复位命令失败，失败代码：" + str(res) + "\n")
        else:
            print("发送DMD-复位命令成功\n", 0)
