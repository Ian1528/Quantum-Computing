...
def camera_Amp_generate(init_targetAmp,camera_intensity_array):
    # This function receives a measured intensity array. This array is ordered by the following procedure:
    # The array starts from the point most closed to the origin, then ordered row by row.
    # Then it will output the corresponding foci amp corresponding to this input intensity array
    # camera_intensity_array的顺序：从最靠近原点的那一行的顶点开始，逐行排序
    camera_intensity=init_targetAmp.copy()
    camera_intensity[np.where(camera_intensity>0)]=np.flip(camera_intensity_array)
...
def SLM_screen_Correct(SLM_screen):
    ...
    LUT_value_for_2pi=223 # 对850nm的LUT数值
    correction=np.array(Image.open('CAL_LSH0804730_850nm.bmp'))
    SLM_screen = SLM_screen.astype(np.uint16)
...
# def SLM_screen_Save(SLM_screen,SLM_screen_Corrected,arraysize,adapt_times,path,Time):
#     # 该函数用来存储SLM相位图，包括未校正的和已经校正的
    
#     SLM_screen=np.array(SLM_screen,dtype='uint8')
#     np.save(path+'/'+Time+'_'+str(arraysize[0])+'x'+str(arraysize[1])+'_adapt'+str(adapt_times)+'_NonCorrected.npy',SLM_screen)
#     SLM_screen_bmp=Image.fromarray(SLM_screen)
#     SLM_screen_bmp.save(path+'/'+Time+'_'+str(arraysize[0])+'x'+str(arraysize[1])+'_adapt'+str(adapt_times)+'_NonCorrected.bmp','bmp')

#     SLM_screen_Corrected=np.array(SLM_screen_Corrected,dtype='uint8')
#     np.save(path+'/'+Time+'_'+str(arraysize[0])+'x'+str(arraysize[1])+'_adapt'+str(adapt_times)+'_Corrected.npy',SLM_screen_Corrected)
#     SLM_screen_Corrected_bmp=Image.fromarray(SLM_screen_Corrected)
#     SLM_screen_Corrected_bmp.save(path+'/'+Time+'_'+str(arraysize[0])+'x'+str(arraysize[1])+'_adapt'+str(adapt_times)+'_Corrected.bmp','bmp')


# def Zernike_Generate(SLMResX, SLMResY, pixelpitch, aperture_radius, ind_Zernike_list, percent_list,isZernikePhaseContinous):
...
#             SLM_aberr_screen = myOberrationCorr.phase_Zernike_continuous(m, n, Plot = True)
